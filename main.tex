...
\begin{itemize}
    ...
    \begin{equation}
        catch\_like(k)\leftarrow catch\_like(k)+\sum_{y=y_0}^{y_N}\dfrac{1}{2}\dfrac{(log(catch\_bio(k,y)+.0001) - log(pred\_catch(k,y)+.0001) )^2}{catch\_bio\_lva(k,y)}
    \end{equation}
    \item Else
    \begin{equation}
        catch\_like(k) \leftarrow catch\_like(k)+catchbiomass\_pen.norm2(log(catch\_bio(k)   
                      +.000001) - log(pred\_catch(k) +.000001)).
    \end{equation}
    Finally
    \begin{equation}
        catch\_like\leftarrow catch\_like.catch\_pen.
    \end{equation}  
\end{itemize}

...
\begin{itemize}
    ...
    \item ${N_{fut}}^s_{y_N,1} = SRecruit(SSB^s_{y_N-rec\_age},cum\_reg(s)+yy\_sr(s,y_N))e^{\epsilon^s_{y_N}}$, where $SRecruit$ is a function of  XXX
...
\begin{itemize}
    ...
    \begin{equation}
        ...
        \end{equation} 
\begin{itemize}
    \item $C^s_{i,j}$ is the total catch of stock $s$ for year $i$ at age $j$. It is used in eq. 6.
    \item $Cat(k,i)_j$ is the catch at age $j$ of fishery $k$ in year $i$. It is used in eq. 6.
...
% \end{equation}
\begin{equation}
    B^{s,r}_0 = SB^s_{reg\_shift(s,r-1)-a_R}
\end{equation}
